...
while k <= 0:
    ...
    # чтобы проверить, является ли введенная строка числом попробуем преобразовать ее в int и будем ловить исключение
    try:
        k = input('Введите сколько попыток дается пользователю на правильный ответ: ')
        k = int(k)
    except ValueError:
...
while k != 0 and flag == 0:
    ...
    while n_answer <= 0:
        ...
        # чтобы проверить, является ли введенная строка числом попробуем преобразовать ее в int и будем ловить исключение
        try:
            n_answer = input() # Пользователь угадывает число
            n_answer = int(n_answer)
        except ValueError:
            print("\033[31m" + "Введите значение целым числом !!!" + "\033[0m")
            logging.error(f'Пользователь ввел не целое число: "{n_answer}" !!!')
            n_answer = 0
            flag = 1
            pass

        if (n_answer < 1 or n_answer > N) and flag == 0:
            print("\033[31m" + f"Введенное значение не попадает в диапазон от 1 до {N} !!!" + "\033[0m")
            logging.error(f'Пользователь ввел число не попадающие в диапазон от 1 до {N}: {n_answer} !!!')
